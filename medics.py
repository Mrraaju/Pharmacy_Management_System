...
class medicine():
    def __init__(self, root):
        ...
        title = tk.Label(self.root, text="Raju Pharmacy Management System",fg="green", bd=4, relief="solid", bg=self.clr(160,160,160), font=("Times New Roman",50,"bold"))
        ...
        medLbl = tk.Label(addFrame, bg=self.clr(150,220,150), text="Medicine name:", font=("Arial",18,"bold"))
    ...
    def addFun(self):
        ...
        if med and p and q and exp:
            ...
            try:
                self.dbFun()
                self.cur.execute("insert into medi(medicine,price,quant,exp) values(%s,%s,%s,%s)",
                                 (med,price,quant,exp))
                self.con.commit()
                tk.messagebox.showinfo("Success",f"Medicine {med} is added Successfuly!")
                self.cur.execute("select * from medi where medicine=%s",med)
    ...
    def dbFun(self):
        self.con = pymysql.connect(host="localhost", user="root", passwd="Raju@123", database="pharmacys")
    ...
    def saleMed(self):
        ...
        try:
            self.dbFun()
            self.cur.execute("select price,quant from medi where medicine=%s",med)
            row = self.cur.fetchone()
            if row:
                if row[1] >=quant:
                    upd = row[1]-quant
                    amount = row[0]*quant
                    self.cur.execute("update medi set quant=%s where medicine=%s",(upd,med))
    ...
    def updMed(self):
        ...
        try:
            self.dbFun()
            self.cur.execute("select quant from medi where medicine=%s",med)
            data = self.cur.fetchone()
            newQuant = data[0]+quant
            self.cur.execute("update medi set quant=%s where medicine=%s",(newQuant,med))
            self.con.commit()
    ...
    def showAll(self):
        try:
            self.dbFun()
            self.cur.execute("select * from medi")
